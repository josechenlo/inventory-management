      ...
      <div class="card">
        <div class="card-header">
          <div class="card-header-left">
            ...
            <button
              class="export-button"
              :title="t('inventory.exportCsvTooltip')"
              @click="exportToCSV"
            >
              <svg
...
export default {
  ...
  setup() {
    ...
    // Escape a value for safe inclusion in a CSV cell (wrap in quotes if it
    // contains commas, quotes, or newlines; double up any inner quotes).
    ...
    // Build a filename-safe timestamp like 2026-07-20-14-55-30
    const getTimestamp = () => {
      const now = new Date();
    ...
    // Export the currently filtered/sorted inventory items to a CSV file
    const exportToCSV = () => {
      const headers = [
      ...
      const rows = filteredItems.value.map((item) => {
        ...
        return [
          ...
          `${currencySymbol.value}${totalValue.toLocaleString(undefined, {
      ...
      // Prepend BOM so Excel opens the file with correct UTF-8 encoding
      ...
      link.href = url;
      link.download = `inventory-${getTimestamp()}.csv`;
